...
void setup(void) 
{
	...
	coos_task_add( rgbs_update, 0, 50 );
...
void rgbs_update(void)
{
	...
	{
		rgb.setPixelColor( cnt, rgb.Color( rgb_colors[0], rgb_colors[1], rgb_colors[2] ));
	}	
	rgb.show();
}
...
void enc_check(void)
{
	for( int cnt=0; cnt<3; cnt++)
	{
		...
		if( val != 0 )
		{
			...
			if( (rgb_colors[cnt] + val) > 255 )
			{
				rgb_colors[cnt] = 255;
			}
			else if( (rgb_colors[cnt] + val) < 0 )
			{
				rgb_colors[cnt] = 0;
			}
			else
